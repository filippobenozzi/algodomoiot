...
## Build firmware ESP32 + Web Flasher

File aggiunti:

- script build: `./build_esp32_firmware.sh`
- firmware sorgente: `esp32_firmware/sheltr_esp32/sheltr_esp32.ino`
- pagina flash: `http://<IP_RASPBERRY>/esp32_flash.html`

Passi:

1. Sul server/progetto esegui:

```bash
chmod +x ./build_esp32_firmware.sh
./build_esp32_firmware.sh
./build_esp32_firmware.sh "NOME_WIFI" "PASSWORD_WIFI"
./build_esp32_firmware.sh "NOME_WIFI" "PASSWORD_WIFI" "esp32:esp32:esp32s3"
```

Se passi `SSID` e `password`, il firmware prova prima la connessione WiFi in modalita STA.
Se la connessione fallisce, passa in fallback AP (`Sheltr-ESP32`).
Il terzo parametro `FQBN` seleziona il target (default: `esp32:esp32:esp32s3`).

2. Apri in Chrome/Edge desktop:

- `http://<IP_RASPBERRY>/esp32_flash.html`

3. Clicca `Install`, seleziona la porta USB dell'ESP32 e avvia il flash.

Note:

- Lo script usa `arduino-cli` e installa/aggiorna il core `esp32:esp32`.
- I binari e il manifest vengono pubblicati in `public/esp32/`.
- Le credenziali WiFi passate nello script entrano nel firmware compilato.
